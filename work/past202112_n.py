...
p = Polygon([Point(*tuple(map(int, input().split()))) for _ in range(n)])
q = Polygon([Point(*tuple(map(int, input().split()))) for _ in range(m)])
r = []

for i in range(n):
    li = Line(p.points[i], p.points[(i+1)%n])
    for j in range(m):
        lj = Line(q.points[j], q.points[(j+1)%m])
        pij = li.cross_point(lj)
        if pij: r.append(pij)
