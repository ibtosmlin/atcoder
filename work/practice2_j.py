...
class SegmentTree(SegTree):
    ...
    def __str__(self):
        """元のリストの値を表示"""
        return self.debug(self, self._d)

    def debug(self, xs, minsize=0):
        strs = [str(x) for x in xs]
        for i in range(self._size, len(xs)):
            strs[i] = f'{strs[i]}({i-self._size})'
        s = max(len(s) for s in strs[self._size:])
